...
public class IbatisGenerate {
    ...
    public static void generate() {
        ...
        //模块名称
        String controllerModule = "pipeline-gate";
        String serviceModule = "pipeline-config";
        String entityModule = "pipeline-common";
        String packageName = "config";
        String[] tableName =  new String[]{"p_config_application","p_cfg_pipeline","p_cfg_pipeline_notification","p_cfg_pipeline_parameter","p_cfg_pipeline_trigger","p_config_application","p_ctg_stage"};
        preCustomPath(controllerModule,serviceModule,entityModule,packageName,customPathInfo);

        FastAutoGenerator.create("jdbc:mysql://localhost:3306/pipeline?useSSL=false&useUnicode=true&characterEncoding=utf-8&serverTimezone=UTC",
                        "root",
                        "eadp2022")
    ...
    private static void preCustomPath(String controllerModule, String serviceModule, String entityModule,String packageName, Map<OutputFile, String> customPathInfo) {
        ...
        // 包名
        String controllerPackageName = "com.ctg.pipeline.controller".replace('.', File.separatorChar);
        String entityPackageName = "com.ctg.pipeline.common.entity".replace('.', File.separatorChar);
        String servicePackageName = ("com.ctg.pipeline."+serviceModule.substring(serviceModule.indexOf("-")+1)+".service").replace('.', File.separatorChar);
        String serviceImplPackageName = ("com.ctg.pipeline."+serviceModule.substring(serviceModule.indexOf("-")+1)+".service.impl").replace('.', File.separatorChar);
        String mapperPackageName = ("com.ctg.pipeline."+serviceModule.substring(serviceModule.indexOf("-")+1)+".dao").replace('.', File.separatorChar);
